...
// 1d. MODO DE OPERAÇÃO
// --------------------------------------------------
// "linha" - 2 cliques definem uma reta (tecla R)
// "triangulo" - 3 cliques definem um triângulo (tecla T)

let modoAtual = "linha";

// Pontos já clicados, aguardando completar a figura
let pontosClicados = [];

function atualizarIndicadorDeModo() {

    const nomeDoModo = (modoAtual === "linha") ? "Linha" : "Triângulo";
    modoAtualSpan.textContent = `Modo: ${nomeDoModo}`;

    const necessarios = (modoAtual === "linha") ? 2 : 3;
    progressoSpan.textContent =
        `Cliques: ${pontosClicados.length}/${necessarios}`;
}
...
// --------------------------------------------------
// 11b. FUNÇÃO: TRAÇAR TRIÂNGULO
// --------------------------------------------------
// Traça as 3 arestas do triângulo usando Bresenham, a partir de 3 vértices.

function tracarTriangulo(p1, p2, p3) {

    const pixels = [];

    pixels.push(...bresenham(p1.x, p1.y, p2.x, p2.y));
    pixels.push(...bresenham(p2.x, p2.y, p3.x, p3.y));
    pixels.push(...bresenham(p3.x, p3.y, p1.x, p1.y));
...
// 12. FUNÇÃO: MUDAR COR
// --------------------------------------------------
// Troca a cor atual pela cor indexada na paleta e redesenha a figura com a cor nova.
...
// --------------------------------------------------
// Todos os pixels da figura atual usam a mesma cor e o mesmo tamanho, então repetimos os valores pra cada vértice antes de subir os buffers.
...
// --------------------------------------------------
// Modo linha: 2 cliques (inicial e final) -> tracarLinha.
// Modo triângulo: 3 cliques seguidos (vértices) -> tracarTriangulo.
...
function mouseClick(event) {
    ...
    const pontosNecessarios = (modoAtual === "linha") ? 2 : 3;

    if (pontosClicados.length === pontosNecessarios) {

        if (modoAtual === "linha") {
            tracarLinha(
                pontosClicados[0].x, pontosClicados[0].y,
                pontosClicados[1].x, pontosClicados[1].y
            );
        } else {
            tracarTriangulo(
                pontosClicados[0],
...
function keyboardClick(event) {
    ...
    switch (event.key) {
        ...
        case "0": mudarCor(0); return;
        case "1": mudarCor(1); return;
        case "2": mudarCor(2); return;
        case "3": mudarCor(3); return;
        case "4": mudarCor(4); return;
        case "5": mudarCor(5); return;
        case "6": mudarCor(6); return;
        case "7": mudarCor(7); return;
        case "8": mudarCor(8); return;
        case "9": mudarCor(9); return;
